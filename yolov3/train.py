...
if __name__ == "__main__":
    ...
    #model.summary()
    #model.load_weights("./yolov3_fore/yolov3-045.h5", by_name=True)
    
    # Callback set
    checkpoint = ModelCheckpoint(os.path.join(config.TENSORBOARD_DIR, config.WEIGHTS_FILE + "-{epoch:03d}.h5"),
                                monitor='val_loss', save_best_only=True, save_weights_only=False, mode='auto', verbose=1)
    ...
    # Train
    model.compile(optimizer=SGD(learning_rate=config.LEARNING_RATE, momentum=0.9, decay=5e-4), loss={'yolo_loss': lambda y_true, y_pred: y_pred})

...
def get_anchors_512(image_size=(512, 512), anchors=[36, 77, 154, 230, 307, 384, 461, 538]):

    if image_size != (512, 512):
        raise ValueError("This anchor need to used (512, 512).")

    features_map_length = [64, 32, 16, 8, 4, 2, 1] 
    variances = [0.1, 0.1, 0.2, 0.2]
    ...
    priors = AnchorBoxes(img_size=image_size, min_size=anchors[4], max_size=anchors[5],aspect_ratios=[2, 3],
                        variances=variances)
    conv8_2_mbox_priorbox = priors.call((features_map_length[4],features_map_length[4]))
    ...
    priors = AnchorBoxes(img_size=image_size, min_size=anchors[6], max_size=anchors[7],aspect_ratios=[2],
                        variances=variances)
    conv10_2_mbox_priorbox = priors.call((features_map_length[6],features_map_length[6]))

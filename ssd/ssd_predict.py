...
class detector(object):
    ...
    def __init__(self, weight_path=None, network=None, groups=3):
        ...
        self._load_weigth(weight_path=weight_path, network=network, groups=groups)
    ...
    def _load_weigth(self, weight_path=None, network=None, groups=3):
        ...
        if network == 'mobilenetv1':
            ...
            raise ValueError("You need network choice `mobilenetv1`, `mobilenetv2`, `mobilenetv3`, `mobilenet_shuffle`,\
                            `shufflenetv1` or `shufflenetv2`")
        
        # load weigth file
        if network == 'shufflenetv1':
            self.model = SSD300(config.IMAGE_SIZE_300, len(self.classes), anchors=config.ANCHORS_SIZE_300, groups=groups)
        else:
